...
run_group="$(TZ=Asia/Seoul date +%m%d_%H%M%S)_codebook_size_ablation"
...
if ((status != 0)); then
    echo "one or more codebook-size ablations failed" >&2
    exit "$status"
fi

uv run kobeni summarize-experiments --run-dir outputs/architecture
echo "all codebook-size ablations completed"
