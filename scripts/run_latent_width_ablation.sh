...
run_group="$(TZ=Asia/Seoul date +%m%d_%H%M%S)_latent_width_ablation"
...
if ((status != 0)); then
    echo "one or more latent-width ablations failed" >&2
    exit "$status"
fi

uv run kobeni summarize-experiments --run-dir outputs/architecture
echo "all latent-width ablations completed"
